...
public class BalloonOverlayView extends FrameLayout {
	...
	public void setData(OverlayItem item) {
		...
		Checkin checkin = ((FacebookCheckinOverlayItem) (item)).checkin;

		layout.setVisibility(VISIBLE);
		if (item.getTitle() != null) {
			title.setVisibility(VISIBLE);
			try {
				Profile checkinUserProfile = FacebookMaster.profile
						.getProfile(checkin.author_uid);

				Profile checkinPlaceProfile = FacebookMaster.profile
						.getProfile(checkin.page_id);

				title.setText(checkinUserProfile.name + "@"
						+ checkinPlaceProfile.name);
		...
		if (item.getSnippet() != null) {
			...
			String postDate = new Date(checkin.timestamp*1000).toLocaleString();
